...
bool SPWFSA01::startup(int mode)
{
	...
	/*set local echo to 0*/
	if (!(_parser.send("AT+S.SCFG=localecho1,%d\r", 0) && delayRecv("OK")))
	{
		debug_if(dbg_on, "SPWF> error local echo set\r\n");
		return false;
	}
	/*reset factory settings*/
	if (!(_parser.send("AT&F") && delayRecv("OK")))
	{
	...
	if (!(_parser.send("AT+S.SCFG=wifi_ht_mode,%d\r", 1) && delayRecv("OK")))
	{
		debug_if(dbg_on, "SPWF> error setting ht_mode\r\n");
		return false;
	}

	if (!(_parser.send("AT+S.SCFG=wifi_opr_rate_mask,0x003FFFCF\r") && delayRecv("OK")))
	{
	...
	if (!(_parser.send("AT+S.SCFG=wifi_mode,%d\r", mode) && delayRecv("OK")))
	{
	...
	if (!(_parser.send("AT&W") && delayRecv("OK")))
	{
...
bool SPWFSA01::reset(void)
{
	...
	while (1) {
		if (delayRecv("+WIND:32:WiFi Hardware Started\r")) {
			return true;
		}
	}
}
...
void SPWFSA01::waitSPWFReady(void)
{
	//wait_ms(200);
	while (1)
		if (_parser.send("AT") && delayRecv("OK"))
...
bool SPWFSA01::connect(const char *ap, const char *passPhrase, int securityMode)
{
	...
	if (!(_parser.send("AT+S.SCFG=wifi_wpa_psk_text,%s", passPhrase) && delayRecv("OK")))
	{
		debug_if(dbg_on, "SPWF> error pass set\r\n");
		return false;
	}
	//AT+S.SSIDTXT=%s\r
	if (!(_parser.send("AT+S.SSIDTXT=%s", ap) && delayRecv("OK")))
	{
		debug_if(dbg_on, "SPWF> error ssid set\r\n");
		return false;
	}
	//AT+S.SCFG=wifi_priv_mode,%d\r
	if (!(_parser.send("AT+S.SCFG=wifi_priv_mode,%d", securityMode) && delayRecv("OK")))
	{
	...
	/*set idle mode (0->idle, 1->STA,3->miniAP, 2->IBSS)*/
	if (!(_parser.send("AT+S.SCFG=wifi_mode,%d\r", 1) && delayRecv("OK")))
	{
		debug_if(dbg_on, "SPWF> error wifi mode set\r\n");
		return false;
	}
	...
	while (1) {
		if ((delayRecv("+WIND:24:WiFi Up:%u.%u.%u.%u", &n1, &n2, &n3, &n4)))
		{
			break;
		}
	}
...
bool SPWFSA01::disconnect(void)
{
	//"AT+S.SCFG=wifi_mode,%d\r"
	/*set idle mode (0->idle, 1->STA,3->miniAP, 2->IBSS)*/
	if (!(_parser.send("AT+S.SCFG=wifi_mode,%d\r", 0) && delayRecv("OK")))
	{
		debug_if(dbg_on, "SPWF> error wifi mode set\r\n");
		return false;
	}
...
const char *SPWFSA01::getIPAddress(void)
{
	...
	if (!(_parser.send("AT+S.STS=ip_ipaddr")
		&& delayRecv("#  ip_ipaddr = %u.%u.%u.%u", &n1, &n2, &n3, &n4)
		&& delayRecv("OK"))) {
...
const char *SPWFSA01::getNetmask()
{
	if (!(_parser.send("AT+CIPSTA?")
		&& delayRecv("+CIPSTA:netmask:\"%15[^\"]\"", _netmask_buffer)
		&& delayRecv("OK"))) {
		return 0;
	}
...
{
	if (!(_parser.send("AT+CIPSTA?")
		&& delayRecv("+CIPSTA:gateway:\"%15[^\"]\"", _gateway_buffer)
		&& delayRecv("OK"))) {
		return 0;
	}
...
const char *SPWFSA01::getMACAddress(void)
{
	...
	if (!(_parser.send("AT+S.GCFG=nv_wifi_macaddr")
		&& delayRecv("#  nv_wifi_macaddr = %x:%x:%x:%x:%x:%x", &n1, &n2, &n3, &n4, &n5, &n6)
		&& delayRecv("OK"))) {
...
int8_t SPWFSA01::getRSSI()
{
	int8_t rssi;
	char bssid[18];

	if (!(_parser.send("AT+CWJAP?")
		&& delayRecv("+CWJAP:\"%*[^\"]\",\"%17[^\"]\"", bssid)
		&& delayRecv("OK"))) {
		return 0;
	}

	if (!(_parser.send("AT+CWLAP=\"\",\"%s\",", bssid)
		&& delayRecv("+CWLAP:(%*d,\"%*[^\"]\",%hhd,", &rssi)
		&& delayRecv("OK"))) {
		return 0;
	}
...
bool SPWFSA01::recv_ap(nsapi_wifi_ap_t *ap)
{
	int sec;
	bool ret = delayRecv("+CWLAP:(%d,\"%32[^\"]\",%hhd,\"%hhx:%hhx:%hhx:%hhx:%hhx:%hhx\",%d", &sec, ap->ssid,
...
int SPWFSA01::delayRecv(const char *response, ...)
{
	va_list args;
...
int32_t SPWFSA01::recv(int id, void *data, uint32_t amount)
{
	...
	if (!(_parser.send("AT+S.SOCKQ=%d", id)  //send a query (will be required for secure sockets)
		&& delayRecv(" DATALEN: %u", &recv_amount)
		&& delayRecv("OK"))) {
		return -2;
	}
	...
	while (delayRecv("+WIND:%d:", &wind_id)) {
		if (wind_id == 58) {
			socket_closed = 1;
			_parser.flush();
		}
...
bool SPWFSA01::close(int id)
{
	...
	for (unsigned i = 0; i < 2; i++) {
		...
		{
			if (delayRecv("ERROR: Pending data")) {
...
int SPWFSA01::settime(time_t ctTime)
{
	_parser.flush();
	//May take a second try if device is busy or error is returned
	for (unsigned i = 0; i < 2; i++) {
...
bool SPWFSA01::gettime(time_t *ctTime)
{
	_parser.flush();
	//May take a second try if device is busy or error is returned
	for (unsigned i = 0; i < 2; i++) {
		if (_parser.send("AT+S.STS=current_time")
			&& delayRecv("#  current_time = %u", ctTime)
			&& delayRecv("OK")) {
			return true;
		}
		else
...
int32_t SPWFSA01::cleanTLScertificate(CertType_t type)
{
	...
	if (_parser.send("AT+S.TLSCERT2=clean,%s", cert_type) && delayRecv("OK")) {
		return true;
	}
	else {
